...
Jerk = new ( function Jerk() {
  var bot
    ...
    , stop_watching_for = _stop_watching_for.bind( this )
    , user_join = _user_join.bind(this)
    , user_leave = _user_leave.bind(this)

  /* ------------------------------ Public Methods ------------------------------ */
  this.addWatchers = function( block ) {
    block( { watch_for: watch_for, stop_watching_for: stop_watching_for, user_join: user_join, user_leave: user_leave} )
  ...
  function _stop_watching_for( pattern ) {
    for ( watcher in watchers ) {
        if (watchers[watcher][0]+'' === pattern+'') {
